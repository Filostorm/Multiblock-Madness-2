...
onEvent("ponder.registry", (event) => {
    event.create(["naturesaura:animal_spawner","naturesaura:birth_spirit"])
    .scene("birth_spirit", "Spirit of Birthing", "kubejs:grass_9x9", (scene, util) => {
        scene.showStructure()
        scene.text(60, "Breeding animals in an area with high levels of Aura has a chance to drop a Spirit of Birthing.").colored(PonderPalette.SLOW)
        scene.idle(10)
        ...
        const babyCow = scene.world.createEntity("cow", [4.5, 1, 4.5],  
        entity => {
        ...
        scene.idle(5)
        scene.world.createItemEntity([4.5, 1, 4.5], util.vector.of(0, 0.4, -0.07), "naturesaura:birth_spirit");
    })
    .scene("birthing_altar", "The Brithing Altar", "kubejs:birthing_altar", (scene, util) => {
        ...
        scene.text(20, "Place the Birthing Altar in the center", [4.5, 2, 4.5]).placeNearTarget()
        scene.idle(30)
    });
