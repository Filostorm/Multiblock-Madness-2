                ...
                if (part.name != 'grit') {
                    ...
                    if (Item.of(`kubejs:${part.name}_${item.material}`) != null) {
                        event.addLang(
                            ...
                            `${partColor[part.grade]}${global.displayNameFunction(item.material, item.ore_name, part.prefix, part.suffix)}`
                        );
                    }
                    if (
                        Item.of(`create:crushed_raw_${item.material}`) != null &&
                        part.name == 'crushed'
                    ) {
                        event.addLang(
...
onEvent('item.tooltip', (tooltip) => {
    ...
    tooltip.addAdvanced('forbidden_arcanus:blood_test_tube', (item, advanced, text) => {
        text.add(1, [
            Text.red('Filled With Blood By Hitting Mobs With The '),
            Text.gold('Mystical Dagger')
        ]);
    });
    tooltip.addAdvanced('summoningrituals:altar', (item, advanced, text) => {
        text.add(1, [
            Text.of('Look At The "Useful Things" Chapter To See What Can Be Summoned').aqua()
        ]);
    });
        ...
        if (item.ore) {
            ...
            if (item.type != 'compound_ore') {
                tooltip.addAdvanced(`kubejs:grit_${item.material}`, (items, advanced, text) => {
                    ...
                    text.add(2, [
                        Text.white(`Can be smelted in: `),
                        Text.yellow(`${nameUpper(smeltingList[item.tier])}`)
                    ]);
                });
            } else {
                tooltip.addAdvanced(`kubejs:grit_${item.material}`, (items, advanced, text) => {
                    text.add(1, [Text.of(`Metal Tier: `).white(), Text.of(`${item.tier}`).gold()]);
                    text.add(2, [
                        Text.white(`Can be smelted into: `),
                        Text.gold(`${nameUpper(item.components[0])}`)
                    ]);
                });
            }
            ...
            if (item.tier <= 1) {
                if (item.type == 'compound_ore') {
                    tooltip.addAdvanced(
                        ...
                        (items, advanced, text) => {
                            text.add(1, [
                                Text.of(`Metal Tier: `).white(),
                                Text.of(`${item.tier}`).gold()
                            ]);
                            text.add(2, [
                                Text.white(`Can be smelted into `),
                                Text.gold(`${nameUpper(item.components[0])} Nuggets`),
                                Text.white(` or combined into Raw Ore`)
                            ]);
                        }
                    );
                } else {
                ...
                if (item.type != 'compound_ore') {
                    ...
                    let smeltingFunction = (tier) => {
                        ...
                        if (tier - 1 > part.grade && part.name != 'grit') {
                            smeltingTooltip = Text.of(
                                `Must be processed further to be smelted or refined.`
                            ).red();
                        } else {
                            smeltingTooltip = [
                                Text.of(`Can be smelted in: `).white(),
                    ...
                    let refiningFunction = (part, partLevel) => {
                        ...
                        if (partLevel == index - 1) {
                            ...
                            if (item.tier - 1 <= partLevel || item.tier - 1 <= partLevel + 1) {
                                refiningStep = [
                                    Text.aqua(` ↗ x${global.refiningMultiplier[partLevel]}`),
                                    Text.white(` ${part}`)
                                ];
                            } else {
                                refiningStep = Text.gray(
                                    ` ↗ x${global.refiningMultiplier[partLevel]} ${part}`
                                );
                            }
                    ...
                    let processingLine1 = (part) => {
                        ...
                        if (part == 'raw') {
                            ...
                            tooltipLine1 = [
                                smeltingDashFunction(item.tier, index - 1),
                                Text.gray(oreProcessingPartList[index - 1])
                            ];
                        } else {
                            tooltipLine1 = [
                                smeltingDashFunction(item.tier, index - 1),
                                Text.gray(oreProcessingPartList[index - 1]),
                                Text.gray(` [${gradeLetter[index - 1]}]`),
                                refiningFunction(oreRefiningPartList[index - 1], index - 1)
                            ];
                        }
                        return tooltipLine1;
                    };
                    ...
                    let processingLine2 = (part) => {
                        ...
                        if (part == 'raw') {
                            tooltipLine2 = [
                                smeltingDashFunction(item.tier, index),
                                Text.of(oreProcessingPartList[index]).white()
                            ];
                        } else {
                            tooltipLine2 = [
                                smeltingDashFunction(item.tier, index),
                                ...
                                refiningFunction(oreRefiningPartList[index], index)
                            ];
                        }
                        return tooltipLine2;
                    };
                    ...
                    let processingLine3 = (part) => {
                        ...
                        if (
                            oreProcessingPartList[oreProcessingPartList.length - 1] ==
                            nameUpper(part)
                        ) {
                            //last item on the list has no next step -2 because grit is last ******moved grit to a seperate list so its -1 now
                        ...
                        tooltip.addAdvanced(name, (items, advanced, text) => {
                            ...
                            if (!tooltip.shift) {
                                ...
                                text.add(4, [
                                    Text.of('Hold ').gray(),
                                    Text.of('Shift ').green(),
                                    Text.of('to see more information.').gray()
                                ]);
                            } else {
                    ...
                    if (part.name == 'raw' || part.name == 'crushed') {
                        mods.forEach((mod) => {
                            if (Item.of(`${mod}:${part.name}_${item.material}`) != null) {
                                ...
                                Item.of(`${mod}:${part.name}_raw_${item.material}`) != null
                            ) {
                    ...
                    let compoundSmeltingFunction = (grade) => {
                        ...
                        if (grade <= 0) {
                            ...
                            smeltingTooltip = Text.red(
                                `Must be sorted into base metals before smelting.`
                            );
                        }
                        return smeltingTooltip;
                    };
                    ...
                    let compoundLine = (line) => {
                        ...
                        if (line <= part.grade + 1) {
                            if (part.grade == 0) {
                                ...
                                if (line == 0) {
                                    //yellow = smeltable
                                    tooltipLine = Text.yellow(
                                        `- ${nameUpper(item.components[line])}`
                                    );
                                } else {
                                ...
                                tooltipLine = [
                                    gradeLetterColor[index],
                                    Text.white(` ${nameUpper(item.components[line])}`)
                                ];
                            }
                        } else if (line == part.grade + 2) {
                            if (line != 8) {
                                // replaces a line with this text
                                tooltipLine = [
                                    Text.white(`Increase `),
                                    Text.green(`Processing`),
                                    Text.white(` for:`)
                                ];
                            } else {
                    ...
                    let compoundLineShift = (line) => {
                        ...
                        if (line <= part.grade + 1) {
                            ...
                            tooltipLine = [
                                ...
                                Text.gray(` ${nameUpper(item.components[line - 1])}`)
                            ];
                        }
                        return tooltipLine;
                    };
            ...
            global.oreRefiningParts.forEach((part, index) => {
                let smeltingFunction = (tier) => {
                    ...
                    if (part.level == 1) {
                        ...
                        smeltingTooltip = Text.of(
                            `Must be refined to level 1 before smelting.`
                        ).aqua();
                    }
                    return smeltingTooltip;
                };
                ...
                let refiningLine1 = (linePart) => {
                    let tooltipLine1 = '';
                    if (
                        oreRefiningPartList[oreRefiningPartList.length - 1] == nameUpper(linePart)
                    ) {
                        ...
                        tooltipLine1 = [
                            ...
                            Text.gray(oreRefiningPartList[index + 1]),
                            Text.gray(` (${index + 1})`)
                        ];
                    }
                    return tooltipLine1;
                ...
                tooltipLine2 = [
                    ...
                    Text.of(oreRefiningPartList[index]).white(),
                    Text.aqua(` (${index})`)
                ];
                //	return tooltipLine2
                //}
