...
onEvent('recipes', (event) => {
    global.newMaterialParts.forEach((item) => {
        if (item.type == 'alloy') {
            ...
            event.remove({
                id: `beyond_earth:smeltery/melting/metal/${item.material}/compresseds`
            });
            ...
            if (item.dust_input != null && item.tier != null) {
                if (item.amount != null) {
                    ...
                    console.log(
                        `[MBM2 ERROR]: ${item.material} needs an amount in MML to get a mixer recipe!`
                    );
                }
            }
            if (item.base && item.fluid_input != null && item.tier <= 3) {
                global.anyFilling(
                    ...
                    `#forge:base_ingots/${item.material}`,
                    Fluid.of(item.fluid_input, item.fluid_amount)
                );
            }
            ...
            if (item.tier <= 1) {
                if (item.amount != null) {
                    if (item.dust_input != null) {
                        ...
                        event.recipes
                            .createMixing(
                                [`${item.amount}x #forge:dusts/${item.material}`],
                                item.dust_input
                            )
                            .id(`mbm2:create/mixing/${item.material}_dust`);
                    }
                    if (item.ingot_input != null) {
                        //Mixing Ingots
                        event.recipes
                            .createMixing(
                                `${item.amount}x #forge:ingots/${item.material}`,
                                item.ingot_input
                            )
                            .heated()
                            .id(`mbm2:create/mixing/${item.material}_ingot`);
                        ...
                        if (item.ingot_input.length == 2) {
                            ...
                            event.recipes
                                ...
                                .id(`mbm2:kiln/${item.material}_ingot`);
                        }
                    }
                } else {
                    console.log(
                        `[MBM2 ERROR]: ${item.material} needs an amount in MML to get create mixing recipes!`
                    );
                }

                //Alloy Dust Smelting
                if (
                    Item.of(`#forge:dusts/${item.material}`) != null &&
                    Item.of(`#forge:ingots/${item.material}`) != null
                ) {
                    event
                        .smelting(`#forge:ingots/${item.material}`, `#forge:dusts/${item.material}`)
                        .id(`mbm2:${item.material}_furnace_smelting`);
                    event
                        .blasting(`#forge:ingots/${item.material}`, `#forge:dusts/${item.material}`)
                        .id(`mbm2:${item.material}_furnace_blasting`);
                }
            }
            ////////////// TIER 2 OR BELOW /////////////////////
            if (item.tier <= 2) {
                if (item.tinkers_input != null) {
                    ...
                    global.tinkersAlloying(
                        ...
                        item.tinkers_input,
                        1000,
                        `mbm2:smeltery/alloys/molten_${item.material}`
                    );
                }
            }
            ////////////// TIER 3 OR BELOW /////////////////////
            if (item.tier <= 3) {
                        ...
                        if (item.base) {
                            ...
                            event.recipes
                                .immersiveengineeringArcFurnace(
                                    `${item.amount}x #forge:base_ingots/${item.material}`,
                                    item.ingot_input[0],
                                    item.ingot_input.slice(1)
                                )
                                .id(`mbm2:arc_furnace/${item.material}_alloying_base_ingot`);
                        } else {
                            //Arc Furnace
                            event.recipes
                                .immersiveengineeringArcFurnace(
                                ...
                                .id(`mbm2:arc_furnace/${item.material}_alloying`);
                        }
                    }
                    if (item.ingot_input.length <= 3) {
                        if (item.base) {
                            ...
                            event.recipes.thermal
                                ...
                                .id(`mbm2:induction/${item.material}_base_ingot`);
                        } else {
                            //Induction
                            event.recipes.thermal
                                .smelter(
                                ...
                                .id(`mbm2:induction/${item.material}_ingot`);
                        }
                    }
                }
                ...
                if (
                    ...
                    event.recipes
                        ...
                        .id(`mbm2:arc_furnace/${item.material}_ingot_from_dust`);
                }
            }
                    ...
                    event.recipes.multiblocked
                        ...
                        .predicate((recipe, recipeLogic) => {
                            ...
                            global.coilPos.forEach((pos) => {
                                let coilName = level
                                    .getBlockState(
                                        controllerTe.getBlockPos().offset(pos.x, pos.y, pos.z)
                                    )
                                    .getBlock();
                                global.coilHeatValues.forEach((material) => {
                                    if (
                                        Block.getBlock(`kubejs:${material.name}_coil`).equals(
                                            coilName
                                        )
                                    ) {
