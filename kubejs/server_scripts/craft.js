...
onEvent('block.right_click', (event) => {
    ...
    if (item.count > 1 && totalsize != [27 || 54]) {
        width = item.count;
    } else {
        width = 9;
    }
    ...
    for (let offsetY = 0; offsetY < maxOffsetY; offsetY++) {
        for (let offsetX = 0; offsetX < maxOffsetX; offsetX++) {
            ...
            let mapRaw = {
                ...
                2: inv.get(offset + 1),
                3: inv.get(offset + 2),
                4: inv.get(offset + 0 + width),
                5: inv.get(offset + 1 + width),
                6: inv.get(offset + 2 + width),
                7: inv.get(offset + 0 + width * 2),
                8: inv.get(offset + 1 + width * 2),
                9: inv.get(offset + 2 + width * 2)
            };
            ...
            switch (removalItem) {
                ...
                case allRemoval: {
                    removeStr = `event.remove({output: ${output}})`;
                    break;
                }
            }
            ...
            switch (modifier) {
                case 'minecraft:stonecutter': {
                    recipe = `event.stonecutting(${output}, ${mapRaw[5]})`;
                    type = 'Stonecutting';
                    break;
                }
                case 'minecraft:campfire': {
                    recipe = `event.campfireCooking(${output}, ${mapRaw[5]})`;
                    type = 'Campfire';
                    break;
                }
                case 'minecraft:smoker': {
                    recipe = `event.smoking(${output}, ${mapRaw[5]})`;
                    type = 'Smoking';
                    break;
                }
                case 'minecraft:blast_furnace': {
                    recipe = `event.blasting(${output}, ${mapRaw[5]})`;
                    type = 'Blasting';
                    break;
                }
                case 'minecraft:furnace': {
                    recipe = `event.smelting(${output}, ${mapRaw[5]})`;
                    type = 'Smelting';
                    break;
                }
                case 'minecraft:glass_pane': {
                    recipe = `event.shapeless(${output}, [${shapelessList.toString()}])`;
                    type = 'Shapeless';
                    break;
                }
                default: {
                    recipe = `event.shaped(${output}, ['${input}'], {${keyMap.toString()}})`;
                }
            }
            ...
            if (removalItem.toString() != 'Item.empty') {
                let removeString;
                if (removalItem == allRemoval)
                    removeString = buildString(index, output, recipe, 'All', true);
                else if (removalItem == typeRemoval)
                    removeString = buildString(index, output, recipe, type, true);
...
function buildString(index, output, recipe, type, removal) {
    ...
    if (removal) {
        hoverText = Component.join('', [
            Component.white(`Removal Recipe of: \n    `),
            Component.white(`${Item.of(output).name.getString()}`),
            Component.white(`\n    Type: ${type.toString()}`)
        ]);
        recipeInfo = Component.white(` - Removal Recipe: `).append(itemName);
    } else {
        hoverText = Component.join(' ', [
            Component.white(`${type}`),
            Component.white('Recipe of:\n    '),
