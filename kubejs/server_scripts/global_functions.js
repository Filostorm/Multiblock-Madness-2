...
global.createRolling = (event, outputItem, amount, inputItem, ID) => {
    event
        .custom({
            ...
            result: Item.of(outputItem, amount)
        })
        .id(ID);
};
...
global.createApplying = (event, outputBlock, inputItem, inputBlock, ID) => {
    event
        .custom({
            ...
            ingredients: [inputBlock.toJson(), inputItem.toJson()],
            results: [
                {
                    item: outputBlock
                }
...
global.anyFilling = (event, output, inputItem, inputFluid) => {
    event.recipes
        .createFilling(output, [inputFluid, inputItem])
        .id(`mbm2:create/filling/${output.split(':')[1]}`);
    event.recipes.thermal
        .bottler(output, [inputItem, inputFluid])
        .id(`mbm2:thermal/bottling/${output.split(':')[1]}`);
};
...
global.arsEnchantment = (event, outputItem, inputItem, pedestals, source, ID, keepInputNbt) => {
    ...
    let pedestalHolder = [];
    pedestals.forEach((itemOnPedestal) => {
        pedestalHolder.push({
