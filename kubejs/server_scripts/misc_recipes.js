...
onEvent('recipes', (event) => {
    ...
    global.casingTable(
        ...
        250,
        40,
        'tconstruct:smeltery/casting/metal/slag/gem_gold_cast'
    );
    global.casingTable(
        event,
        'tconstruct:casts/single_use/gem',
        true,
        'thermal:slag',
        'forge:molten_slag',
        250,
        40,
        'tconstruct:smeltery/casting/metal/slag/gem_sand_cast'
    );
    ...
    global.casingBasin(
        ...
        1000,
        120,
        'tconstruct:smeltery/casting/metal/slag/block'
    );
    ...
    global.casingBasinCast(
        ...
        'forge:sheetmetals',
        true,
        'kubejs:sheetmetal_cast',
        'forge:molten_steel',
        180,
        200,
        'tconstruct:smeltery/casting/sheetmetal_cast'
    );
...
*/
    ...
    event
        .shaped('scannable:scanner', ['A A', 'CDC', 'EEE'], {
    ...
    event
        .shaped('2x scannable:blank_module', ['AAA', 'ABA', 'CDC'], {
    ...
    // [| Wood Scaffolding |] //
    event
        .shaped('3x kubejs:wood_scaffolding', ['AAA', ' C ', 'C C'], {
            A: '#minecraft:planks',
            C: '#forge:rods/wooden'
        })
    ...
    // [| Summoning Altar |] //
    event
        .shaped('summoningrituals:altar', ['ABC', 'DED', 'EFE'], {
    ...
    event
        .shaped('4x druidcraftrg:beam', [' L ', 'LSL', ' L '], {
    ...
    // Soul Torch
    event
        .shapeless('4x soul_torch', ['torch', 'torch', 'torch', 'torch', 'soul_sand'])
        .id('mbm2:soul_torch');
    ...
    event
        .shapeless(`sticky_piston`, [glue, 'piston'])
        .id(`mbm2:super_sticky_piston`)
        .damageIngredient(glue)
        .keepIngredient(glue);
    ...
    let compressedBlocks = [
        // only vanilla items with this script right now
        'cobblestone',
        'cobbled_deepslate',
        'netherrack'
    ];
    ...
    compressedBlocks.forEach((block) => {
        event
            .shaped(`kubejs:1x_compressed_${block}`, [`CCC`, `CCC`, `CCC`], {
                C: `${block}`
            })
            .id(`mbm2:1x_compressed_${block}`);
        event
            .shapeless(`9x ${block}`, [`kubejs:1x_compressed_${block}`])
            .id(`mbm2:1x_uncompressed_${block}`);
        event
            .shaped(`kubejs:2x_compressed_${block}`, [`CCC`, `CCC`, `CCC`], {
                C: `kubejs:1x_compressed_${block}`
            })
            .id(`mbm2:2x_compressed_${block}`);
        event
            .shapeless(`9x kubejs:1x_compressed_${block}`, [`kubejs:2x_compressed_${block}`])
            .id(`mbm2:2x_uncompressed_${block}`);
        event
            .shaped(`kubejs:3x_compressed_${block}`, [`CCC`, `CCC`, `CCC`], {
                C: `kubejs:2x_compressed_${block}`
            })
            .id(`mbm2:3x_compressed_${block}`);
        event
            .shapeless(`9x kubejs:2x_compressed_${block}`, [`kubejs:3x_compressed_${block}`])
            .id(`mbm2:3x_uncompressed_${block}`);
    });
    //thermal filter augment
    event.remove({ id: 'thermal:augments/item_filter_augment' });
    event
        .shaped('thermal:item_filter_augment', [' T ', 'TFT', ' T '], {
            T: '#forge:nuggets/tin',
    ...
    event
        .shaped('thermal:fluid_filter_augment', [' B ', 'BFB', ' B '], {
    ...
    event.recipes
        .createSequencedAssembly(
            ...
            [
                // input.
                event.recipes.createDeploying('kubejs:incomplete_titanium_plated_obsidian', [
                    'kubejs:incomplete_titanium_plated_obsidian',
                ...
                event.recipes.createPressing('kubejs:incomplete_titanium_plated_obsidian', [
                    'kubejs:incomplete_titanium_plated_obsidian'
                ])
            ]
        )
        .transitionalItem('kubejs:incomplete_titanium_plated_obsidian')
        .loops(2)
        .id('mbm2:titanium_plated_obsidian');

    //Ender Tank and Ender Chest
    event.remove({ id: 'enderstorage:ender_chest' });
    event.remove({ id: 'enderstorage:ender_tank' });
    event
        .shaped('enderstorage:ender_chest', ['ABA', 'CDC', 'AEA'], {
            A: 'extendedcrafting:ender_ingot',
            B: 'tconstruct:enderman_head',
            C: 'kubejs:titanium_plated_obsidian',
            D: 'cloudstorage:static_cloud_chest',
            E: 'minecraft:ender_pearl'
        })
        .id('mbm2:ender_chest');
    event
        .shaped('enderstorage:ender_tank', ['ABA', 'CDC', 'AEA'], {
            A: 'extendedcrafting:ender_ingot',
            B: 'tconstruct:enderman_head',
            C: 'kubejs:titanium_plated_obsidian',
            D: 'beyond_earth:steel_tank',
            E: 'minecraft:ender_pearl'
        })
        .id('mbm2:ender_tank');
    ...
    event
        .shaped(
            Item.of(
                ...
                '{upgrade_info:{key:"compactmachines:chunkloader"}}'
            ),
            ['ABA', 'BCB', 'ABA'],
            {
                A: 'compactmachines:wall',
                B: 'compactcrafting:field_projector',
                C: 'compactmachines:personal_shrinking_device'
            }
        )
        .id('mbm2:chunkloader_upgrade');
    ...
    event.recipes
        .createSequencedAssembly(
            ...
            Item.of('#forge:storage_blocks/compressed_steel'),
            [
                // input.
                event.recipes.createPressing('kubejs:blank_mold', ['kubejs:blank_mold']),
                event.recipes.createFilling('kubejs:blank_mold', [
                    'kubejs:blank_mold',
                    Fluid.of('lava', 1000)
                ]),
                event.recipes.createDeploying('kubejs:blank_mold', [
                    'kubejs:blank_mold',
                    '#forge:bolts'
                ]),
                event.recipes.createPressing('kubejs:blank_mold', ['kubejs:blank_mold'])
        ...
        .transitionalItem('kubejs:blank_mold')
        .loops(1)
        .id(`mbm2:bolt_mold`);
    ...
    global.powahEnergizing(
        event,
        [Item.of('cloudstorage:cloud').toJson()],
        Item.of('cloudstorage:static_cloud'),
        25000,
        'mbm2:cloudstorage/static_cloud'
    );

    //Arcane Sand
    event
        .shapeless('8x kubejs:arcane_sand', [
    ...
    event
        .shaped('minecraft:repeater', ['ABA', 'CCC'], {
            A: 'minecraft:redstone_torch',
            B: 'minecraft:redstone',
            C: 'projectred_core:plate'
        })
        .id('mbm2:stone_plate_repeater');
    event
        .shaped('minecraft:repeater', ['A A', 'BAB', 'CCC'], {
            A: 'minecraft:redstone',
            B: 'minecraft:stick',
            C: 'projectred_core:plate'
        })
        .id('mbm2:stone_plate_repeater_alt');
    event
        .shaped('minecraft:comparator', [' A ', 'ABA', 'CCC'], {
            A: 'minecraft:redstone_torch',
            B: 'minecraft:quartz',
            C: 'projectred_core:plate'
        })
        .id('mbm2:stone_plate_comparator');

    //Flowers Begone (AKA: Azalea Shearing)
    event
        .shapeless('ecologics:azalea_log', ['ecologics:flowering_azalea_log', '#forge:shears'])
        .damageIngredient('#forge:shears', 1)
        .id('mbm2:remove_azalea_flowers');
    event
        .custom({
    ...
    event
        .shaped('darkutils:tome_enchanting', [' A ', 'BCB', ' D '], {
            A: 'ars_nouveau:bookwyrm_charm',
            B: 'create:experience_nugget',
            C: 'tombstone:book_of_disenchantment',
            D: 'thermal:xp_crystal'
        })
        .id('mbm2:darkutil_enchant_tome');

    //Travel Anchor
    event.remove({ output: 'travel_anchors:travel_anchor' });
    event
        .shaped('travel_anchors:travel_anchor', ['CIC', 'IFI', 'CIC'], {
    ...
    //Molten Core
    event
        .shaped('4x reliquary:molten_core', ['BBB', 'BRB', 'BBB'], {
    ...
    global.tinkersMeltingPlain(
        ...
        250,
        Ingredient.of('reliquary:molten_core').toJson(),
        900,
        240,
        `mbm2:smeltery/melting/molten_core`
    );
    ...
    global.naturesauraAltar(
        ...
        5000,
        80,
        'mbm2:conversion/rabbit_hide'
    );

    //Glow Goop alt recipe
    global.arsEnchantment(
        event,
        Item.of('naturalist:glow_goop'),
        Item.of('minecraft:slime_ball'),
        [Item.of('minecraft:glow_berries'), Item.of('minecraft:glow_berries')],
        0,
        'mbm2:glow_goop_alt'
    );
    ...
    event
        .shaped('#forge:storage_blocks/ruby', ['AAA', 'AAA', 'AAA'], { A: '#forge:gems/ruby' })
        .id('mbm2:crafting/ruby_block');
    event
        .shaped('#forge:storage_blocks/sapphire', ['AAA', 'AAA', 'AAA'], {
            A: '#forge:gems/sapphire'
        })
        .id('mbm2:crafting/sapphire_block');
    event
        .shapeless('9x #forge:gems/ruby', ['#forge:storage_blocks/ruby'])
        .id('mbm2:crafting/ruby_block_to_ruby');
    event
        .shapeless('9x #forge:gems/sapphire', ['#forge:storage_blocks/sapphire'])
        .id('mbm2:crafting/sapphire_block_to_sapphire');
    ...
    global.ieCrusher(
        ...
        [
            { chance: 0.25, output: Item.of('#forge:gems/amethyst').toJson() },
            { chance: 0.05, output: Item.of('apotheosis:gem_dust').toJson() }
        ],
        { item: `ars_nouveau:source_gem` }, //Input
        6000,
        `mbm2:crushing/gem_dust_from_source_gem`
    );

    // Modular Rourters
    event.remove({ output: '2x modularrouters:modular_router' });
    event
        .shaped('2x modularrouters:modular_router', ['ABC', 'BDB', 'CBA'], {
            A: 'thermal:redstone_servo',
            B: 'botania:livingrock',
            C: 'kubejs:tier_2_mechanical_alloy_robot_arm',
            D: 'kubejs:titanium_scaffolding'
        })
        .id('mbm2:crafting/modular_router');
    event.remove({ output: 'modularrouters:blank_module' });
    event
        .shaped('modularrouters:blank_module', [' A ', 'BCB', 'DED'], {
            A: 'extendedcrafting:black_iron_slate',
            B: 'solarflux:blank_upgrade',
            C: 'kubejs:mana_diamond_dust',
            D: 'kubejs:tier_2_electrical_alloy_spool',
            E: 'pneumaticcraft:programming_puzzle'
        })
        .id('mbm2:crafting/blank_module');
    event.remove({ output: 'modularrouters:blank_upgrade' });
    event
        .shaped('modularrouters:blank_upgrade', ['ABC', 'DEB', ' DA'], {
            A: 'kubejs:tier_2_electrical_alloy_spool',
            B: 'solarflux:blank_upgrade',
            C: 'compressedcreativity:brass_coated_upgrade_matrix',
            D: 'extendedcrafting:black_iron_slate',
            E: 'kubejs:mana_diamond_dust'
        })
        .id('mbm2:crafting/blank_upgrade');
    ...
    event
        .shaped('2x reliquary:frozen_core', ['ABA', 'BCB', 'ABA'], {
            A: 'minecraft:packed_ice',
            B: 'minecraft:snow_block',
            C: 'ars_nouveau:ritual_awakening'
        })
        .id('mbm2:crafting/frozen_core_alt');
    ...
    event
        .custom({
            ...
            ingredients: [
                { fluid: 'minecraft:water', amount: 3500 },
                { tag: 'forge:fine_dusts/vincyte', count: 1 },
                { item: 'kubejs:crystal_slag', count: 1 }
            ],
            result: [{ tag: 'forge:gems/sapphire', count: 1 }]
        })
