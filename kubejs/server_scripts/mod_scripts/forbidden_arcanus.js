...
onEvent('recipes', (event) => {
    ...
    event
        .shaped('forbidden_arcanus:soul_extractor', ['J  ', 'NNB', 'Q  '], {
...
*/
    ...
    event
        .shapeless(`4x forbidden_arcanus:corrupti_dust`, [
        ...
        .id(`mbm2:forbidden_arcanus/corrupti_dust`);

    //Arcane Crystal Dust speck
    event
        .shapeless(`9x forbidden_arcanus:arcane_crystal_dust_speck`, [
            'forbidden_arcanus:arcane_crystal_dust'
        ])
        .id(`mbm2:arcane_dust_speck`);
    ...
    event.recipes
        .createHaunting('minecraft:soul_sand', 'forbidden_arcanus:soulless_sand')
        .id('mbm2:haunting/resouling_soul_sand');

    //Test Tube Blood Vial fix and filling
    event.remove({ output: 'forbidden_arcanus:test_tube' });
    event
        .shapeless(Item.of('forbidden_arcanus:blood_test_tube', '{Blood:0}'), [
            'forbidden_arcanus:rune',
            'minecraft:glass_bottle'
        ])
        .id(`mbm2:forbidden_arcanus/blood_test_tube`);
    ...
    Object.entries(gavelTypes).forEach(([material, max_uses]) => {
        event
            .shapeless(
                Item.of(
                    `forbidden_arcanus:${material}_blacksmith_gavel`,
                    `{RemainingRitualUses:${max_uses}}`
                ),
                [
    ...
    event.replaceInput(
        { id: 'forbidden_arcanus:golden_dragon_scale' },
        'forbidden_arcanus:dragon_scale',
        'forbidden_arcanus:silver_dragon_scale'
    );
    event.replaceInput(
        { id: 'forbidden_arcanus:silver_dragon_scale' },
        '#forge:ingots/iron',
        '#forge:ingots/silver'
    );
});
