...
onEvent('recipes', (event) => {
    let remove = ['hostilenetworks:living_matter/extraterrestrial/nether_star'];
    ...
    //Casing
    global.elementalcraftBinding(
        event,
        'earth',
        2500,
        'kubejs:captured_soul',
        [
            Item.of('spirit:soul_crystal_shard').toJson(),
            Item.of('forbidden_arcanus:soul').toJson(),
            Item.of('apotheosis:gem_dust').toJson(),
            Item.of('forbidden_arcanus:aureal_bottle').toJson()
        ],
        'mbm2:captured_soul'
    );

    event
        .shaped('kubejs:haunted_casing', ['RPR', 'PSP', 'RPR'], {
    ...
    event.recipes.tconstruct
        .casting_basin('kubejs:hostile_casing', 'tconstruct:blood', 500)
        .cast('kubejs:haunted_casing')
        .consumeCast()
        .coolingTime(250)
        .id(`mbm2:casting/hostile_casing`);

    //Sim chamber
    event.remove({ output: 'hostilenetworks:sim_chamber' });
    event
        .shaped('hostilenetworks:sim_chamber', ['CEC', 'PHG', 'DDD'], {
    ...
    event
        .shaped('hostilenetworks:loot_fabricator', ['PEP', 'CHG', 'PCP'], {
    ...
    //Deep Learner
    event.recipes
        .createPressing('tconstruct:obsidian_pane', 'obsidian')
        .id(`kubejs:pressing/obsidian_pane`);
    event.recipes
        .createCutting('kubejs:deep_learner_base', ['tconstruct:obsidian_pane'])
        .processingTime(400)
        .id(`kubejs:cutting/deep_learner_base`);

    event.remove({ output: 'hostilenetworks:deep_learner' });
    event
        .shaped('hostilenetworks:deep_learner', ['ORO', 'RBG', 'ORO'], {
        ...
        event
            .shaped(
                Item.of(
                    ...
                    `{data_model:{data:6,id:"hostilenetworks:${item.mob}"}}`
                ),
                ['PDP', 'PMP', 'PDP'],
                {
    ...
    event
        .shaped('8x hostilenetworks:empty_prediction', ['DGB', 'GPG', 'BGD'], {
