...
onEvent('recipes', (event) => {
    ...
    event.replaceInput(
        ...
        'iron_ingot',
        Item.of('#forge:ingots/platinum')
    );

    //Tier 1 Frame
    event.remove({ output: `industrialforegoing:machine_frame_pity` });
    event.shaped('industrialforegoing:machine_frame_pity', ['AAA', 'BCB', 'AAA'], {
        A: 'immersiveengineering:treated_wood_horizontal',
        B: '#forge:plates/damascus_steel',
    ...
    event.replaceInput(
        ...
        'iron_ingot',
        'kubejs:stainless_steel_plating'
    );
    ...
    let minDepth = [-64, -14, 36, 86, 136, 186];
    let maxDepth = [0, 50, 100, 150, 200, 255];

    let lensColors = [orangeLens, magentaLens, redLens, blueLens, grayLens, purpleLens];
    ...
    event
        .shaped('industrialforegoing:stasis_chamber', ['AAA', 'BCB', 'DED'], {
            A: 'minecraft:soul_sand',
            B: Item.of('hostilenetworks:prediction', '{data_model:{id:"hostilenetworks:wither"}}'),
            C: 'industrialforegoing:machine_frame_advanced',
            D: 'kubejs:tainted_gold_gear',
            E: 'mekanism:dimensional_stabilizer'
        })
