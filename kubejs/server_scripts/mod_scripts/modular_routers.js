onEvent('recipes', (event) => {
    ...
    event.shaped('3x modularrouters:blank_module', [' A ', 'ABA', 'CCC'], {
        A: 'kubejs:osmium_plating',
        B: 'laserio:overclocker_card',
        C: 'kubejs:damascus_steel_gear'
    });
    event.remove({ output: '2x modularrouters:blank_upgrade' });
    event.shaped('2x modularrouters:blank_upgrade', ['ABB', ' CB', '  A'], {
        A: 'kubejs:damascus_steel_gear',
        B: 'kubejs:osmium_plating',
        C: 'laserio:overclocker_card'
    });
    event.remove({ output: '2x modularrouters:modular_router' });
    event.shaped('2x modularrouters:modular_router', ['ABA', 'CDC', 'ABA'], {
        A: 'modularrouters:augment_core',
        B: 'kubejs:tier_2_electrical_alloy_wire_coil',
        C: 'kubejs:tier_2_mechanical_alloy_robot_arm',
        D: 'laserio:laser_node'
    });
    event.remove({ output: '2x modularrouters:augment_core' });
    event.shapeless('2x modularrouters:augment_core', [
    ...
    event.shapeless(
        Item.of('modularrouters:sender_module_3', '{modularrouters:{ModuleFilter:{}}}'),
        [
            Item.of('modularrouters:sender_module_2', '{modularrouters:{ModuleFilter:{}}}'),
            'enderstorage:ender_chest'
        ]
    );
    event.shapeless('modularrouters:range_up_augment', [
    ...
    event.shaped(
        Item.of(
            'modularrouters:energy_output_module',
            '{modularrouters:{Direction:"NONE",ModuleFilter:{}}}'
        ),
        ['ABA', 'BCB', 'ADA'],
        {
            A: 'powah:energy_cable_niotic',
            B: 'powah:capacitor_niotic',
            C: 'modularrouters:blank_module',
            D: 'laserio:card_energy'
        }
    );
    event.remove({ output: '2x modularrouters:speed_upgrade' });
    event.shaped('2x modularrouters:speed_upgrade', [' A ', 'ABA', ' A '], {
        A: 'ae2:speed_card',
    ...
    event.shaped(
        ...
        [' A ', 'BCB', ' D '],
        {
            A: 'naturesaura:placer',
            B: 'engineersdecor:factory_placer',
            C: 'modularrouters:blank_module',
            D: 'integratedtunnels:part_exporter_world_block'
        }
    );
    event.remove({ output: 'modularrouters:bulk_item_filter' });
    event.shapeless('modularrouters:bulk_item_filter', [
        'modularrouters:blank_module',
        Item.of('laserio:filter_basic', '{allowList:1b,compareNBT:0b,inv:{Items:[],Size:15}}')
    ]);
    event.shaped(
        Item.of(
            'modularrouters:fluid_module',
            '{modularrouters:{Direction:"NONE",FluidDir:0b,MaxTransfer:1000,ModuleFilter:{}}}'
        ),
        [' A ', 'BCB', ' D '],
        {
            ...
            B: 'pipez:fluid_pipe',
            C: 'modularrouters:blank_module',
            D: 'laserio:card_fluid'
        }
    );
    event.shaped(
        Item.of(
            'modularrouters:fluid_module_2',
            '{modularrouters:{FluidDir:0b,MaxTransfer:1000,ModuleFilter:{}}}'
        ),
        ['AB ', '   ', '   '],
        {
            A: Item.of(
            ...
            B: Item.of('industrialforegoing:range_addon11', '{TitaniumAugment:{Range:11.0f}}')
        }
    );
    event.remove({ output: 'modularrouters:fluid_upgrade' });
    event.shaped('modularrouters:fluid_upgrade', [' A ', 'BCB', ' A '], {
        A: 'immersiveengineering:fluid_pipe',
        B: 'pneumaticcraft:small_tank',
        C: 'modularrouters:blank_upgrade'
    });
    event.remove({ output: 'modularrouters:stack_upgrade' });
    event.shapeless('modularrouters:stack_upgrade', [
        'modularrouters:blank_upgrade',
    ...
    event.shaped(
        ...
        [' A ', 'BCB', ' D '],
        {
            ...
            B: 'engineersdecor:small_block_breaker',
            C: 'modularrouters:blank_module',
