...
onEvent('recipes', (event) => {
    ...
    global.newMaterialParts.forEach((item) => {
        if (item.type == 'gem') {
            console.log(
                Item.of(`#forge:gems/${item.material}`) + `: tier ${item.tier} ${item.type}`
            );
        } else {
            console.log(
                Item.of(`#forge:ingots/${item.material}`) + `: tier ${item.tier} ${item.type}`
            );
        }
        ...
        if (Item.of(`#forge:plates/${item.material}`) != null && item.material != 'plastic') {
            if (item.fluid_id != null) {
                //Plate Casting
                event.remove({
                    id: `tconstruct:smeltery/casting/metal/${item.material}/plate_gold_cast`
                });
                event.remove({
                    id: `tconstruct:smeltery/casting/metal/${item.material}/plate_sand_cast`
                });
                event.recipes.tconstruct
                    .casting_table(Item.of(`#forge:plates/${item.material}`), item.fluid_id, 90)
                    .multiUseCast('plate')
                    .coolingTime(60)
                    .id(`mbm2:smeltery/casting/metal/${item.material}/plate_gold_cast`);
                event.recipes.tconstruct
                    .casting_table(Item.of(`#forge:plates/${item.material}`), item.fluid_id, 90)
                    .singleUseCast('plate')
                    .coolingTime(60)
                    .id(`mbm2:smeltery/casting/metal/${item.material}/plate_sand_cast`);
            ...
            //Create Plates
            event.recipes
                .createPressing(`#forge:plates/${item.material}`, `#forge:ingots/${item.material}`)
                .id(`create:pressing/${item.material}_ingot`);

            //Immersive Plates
            event.recipes
                .immersiveengineeringMetalPress(
                    `#forge:plates/${item.material}`,
                    `#forge:ingots/${item.material}`,
                    'immersiveengineering:mold_plate'
                )
                .id(`immersiveengineering:metalpress/plate_${item.material}`);

            //Thermal Plates
            event.recipes.thermal
                .press(`2x #forge:plates/${item.material}`, `3x #forge:ingots/${item.material}`)
                .id(`thermal:machines/press/press_${item.material}_ingot_to_plate`);
            ...
            if (Item.of(`#forge:sheets/${item.material}`) != null) {
                ...
                event.recipes
                    .createCutting(
                        `12x #forge:sheets/${item.material}`,
                        `#forge:storage_blocks/${item.material}`
                    )
                    .id(`mbm2:create_cutting/${item.material}_sheet`);
                ...
                event
                    .custom({
                        ...
                        secondaries: [
                            {
                                output: Item.of(`#forge:dusts/${item.material}`).toJson(),
                                stripping: 'false'
                            }
                        ],
                ...
                event.recipes
                    ...
                    .id(`mbm2:mek_sawmill/sheet_${item.material}`);
            }
            ////////////////PLATING///////////////
            if (Item.of(`#forge:platings/${item.material}`) != null) {
                ...
                event.recipes
                    .createPressing(
                        `#forge:platings/${item.material}`,
                        `#forge:storage_blocks/${item.material}`
                    )
                    .id(`mbm2:pressing/${item.material}_plating`);

                //Immersive Plating
                event.recipes
                    .immersiveengineeringMetalPress(
                        `#forge:platings/${item.material}`,
                        `4x #forge:plates/${item.material}`,
                        'immersiveengineering:mold_plate'
                    )
                    .id(`mbm2:metalpress/plating_${item.material}`);
                ...
                if (Item.of(`#forge:reinforced_platings/${item.material}`) != null) {
                    ...
                    event.recipes
                        .immersiveengineeringMetalPress(
                            `#forge:reinforced_platings/${item.material}`,
                            `4x #forge:platings/${item.material}`,
                            'immersiveengineering:mold_plate'
                        )
                        .id(`mbm2:metalpress/reinforced_platings_${item.material}`);
                }
                ////////////////ROCKET FINS///////////////
                if (Item.of(`#forge:rocket_fins/${item.material}`) != null) {
                    //Hand Crafted Rocket Fins
                    event
                        .shaped(`#forge:rocket_fins/${item.material}`, [' I ', 'PIP', 'GIG'], {
                ...
                if (Item.of(`#forge:rocket_nose_cones/${item.material}`) != null) {
                    ...
                    event
                        .shaped(
                            `#forge:rocket_nose_cones/${item.material}`,
                            [' T ', ' I ', 'IGI'],
                            {
                                G: `#forge:platings/${item.material}`,
                                I: `#forge:ingots/${item.material}`,
            ...
            if (Item.of(`#forge:components/${item.material}`) != null) {
                ...
                event
                    .shaped(`#forge:components/${item.material}`, ['P P', ' I ', 'P P'], {
                ...
                global.ieBlueprint(
                    ...
                    Item.of(`#forge:components/${item.material}`),
                    [
                        { count: 2, base_ingredient: { tag: `forge:plates/${item.material}` } },
                        { tag: `forge:ingots/copper` }
                    ],
                    `mbm2:${item.material}_component`
                );

                //Cheaper recipe
                event.recipes.multiblocked
                    .multiblock('mechanical_crafting')
            ...
            if (Item.of(`#forge:casings/${item.material}`) != null) {
                ...
                event
                    .shaped(`#forge:casings/${item.material}`, ['PPP', 'P P', 'PPP'], {
                        P: `#forge:plates/${item.material}`
                    })
            ...
            if (Item.of(`#forge:sheetmetals/${item.material}`) != null) {
                if (item.fluid_id != null) {
                    event.remove({
                        id: `tconstruct:smeltery/melting/metal/${item.material}/sheetmetal`
                    });
                    global.tinkersMeltingPlain(
                        event,
                        item.fluid_id,
                        90,
                        Item.of(`#forge:sheetmetals/${item.material}`).toJson(),
                        800,
                        142,
                        `mbm2:smeltery/melting/sheetmetal/${item.material}`
                    );
                }
                event
                    .shaped(`2x #forge:sheetmetals/${item.material}`, [' P ', 'P P', ' P '], {
                        P: `#forge:plates/${item.material}`
                    })
                ...
                if (item.fluid_id != null) {
                    //Sheetmetal Casting
                    global.casingBasinCast(
                        event,
                        'forge:sheetmetal_cast',
                        false,
                        `#forge:sheetmetals/${item.material}`,
                        `forge:molten_${item.material}`,
                        90,
                        80,
                        `mbm2:smeltery/casting/metal/${item.material}/sheetmetal`
                    );
                }
            }
        }
        ...
        if (Item.of(`#forge:gears/${item.material}`) != null) {
            ...
            if (Item.of(`#forge:rods/${item.material}`) != null) {
                event
                    .shaped(`#forge:gears/${item.material}`, [' P ', 'PRP', ' P '], {
                        P: `#forge:plates/${item.material}`,
                ...
                if (Item.of(`#forge:cog_blocks/${item.material}`) != null) {
                    event
                        .shaped(`#forge:cog_blocks/${item.material}`, ['RGR', 'G G', 'RGR'], {
                            R: `#forge:rods/${item.material}`,
                    ...
                    if (Item.of(`#forge:scaffoldings/${item.material}`) != null) {
                        event
                            .shaped(`#forge:cog_blocks/${item.material}`, [' G ', 'GSG', ' G '], {
                    ...
                    event.recipes.multiblocked
                        .multiblock('mechanical_crafting')
                        .inputItems([
                            `3x #forge:gears/${item.material}`,
                            `3x #forge:rods/${item.material}`
                        ])
            ...
            if (item.fluid_id != null) {
                //Gear Casing
                event.remove({
                    id: `tconstruct:smeltery/casting/metal/${item.material}/gear_gold_cast`
                });
                event.remove({
                    id: `tconstruct:smeltery/casting/metal/${item.material}/gear_sand_cast`
                });
                //let gearCoolingTime = parseInt(((item.tier + 1)* partAmounts.gear)/3)
                event.recipes.tconstruct
                    .casting_table(Item.of(`#forge:gears/${item.material}`), item.fluid_id, 360)
                    .multiUseCast('gear')
                    .coolingTime(120)
                    .id(`mbm2:smeltery/casting/metal/${item.material}/gear_gold_cast`);
                event.recipes.tconstruct
                    .casting_table(Item.of(`#forge:gears/${item.material}`), item.fluid_id, 360)
                    .singleUseCast('gear')
                    .coolingTime(120)
                    .id(`mbm2:smeltery/casting/metal/${item.material}/gear_sand_cast`);
            ...
            event.recipes
                .immersiveengineeringMetalPress(
                    `#forge:gears/${item.material}`,
                    `4x ${gearPressedMaterial}`,
                    'immersiveengineering:mold_gear'
                )
                .id(`immersiveengineering:metalpress/gear_${item.material}`);

            //Thermal Gears
            event.recipes.thermal
                .press(`#forge:gears/${item.material}`, [
                    `5x ${gearPressedMaterial}`,
                    'thermal:press_gear_die'
                ])
                .id(`thermal:machines/press/press_${item.material}_ingot_to_gear`);
            ...
            if (Item.of(`#forge:interlocking_components/${item.material}`) != null) {
                if (Item.of(`#forge:gears/${item.material}`) != null) {
                    if (Item.of(`#forge:wires/${item.material}`) != null) {
                        event
                            .shaped(
                                `#forge:interlocking_components/${item.material}`,
                                ['G G', ' W ', 'G G'],
                                {
                    ...
                    global.ieBlueprint(
                        event,
                        'interlocking_components',
                        Item.of(`#forge:interlocking_components/${item.material}`),
                        [{ count: 4, base_ingredient: { tag: `forge:gears/${item.material}` } }],
                        `mbm2:${item.material}_interlocking_component`
                    );
                    ...
                    console.log(
                        `${item.material}` +
                            'needs gears enabled to make an interlocking component recipe'
                    );
                }
            }
            ...
            if (Item.of(`#forge:robot_arms/${item.material}`) != null) {
                if (
                    Item.of(`#forge:wires/${item.material}`) != null &&
                    Item.of(`#forge:rods/${item.material}`) != null
                ) {
                    event
                        .shaped(`#forge:robot_arms/${item.material}`, ['ABA', 'CDA', 'E A'], {
        ...
        if (Item.of(`#forge:rods/${item.material}`) != null) {
            if (Item.of(`#forge:ingots/${item.material}`) != null) {
                //Create Rods
                global.createRolling(
                    event,
                    `#forge:rods/${item.material}`,
                    1,
                    `forge:ingots/${item.material}`,
                    `createaddition:rolling/${item.material}_ingot`
                );

                //Immersive Rods
                event.recipes
                    .immersiveengineeringMetalPress(
                        `2x #forge:rods/${item.material}`,
                        `#forge:ingots/${item.material}`,
                        'immersiveengineering:mold_rod'
                    )
                    .id(`immersiveengineering:metalpress/rod_${item.material}`);
            }

            if (item.fluid_id != null) {
                //Rod Casing
                event.remove({
                    id: `tconstruct:smeltery/casting/metal/${item.material}/rod_gold_cast`
                });
                event.remove({
                    id: `tconstruct:smeltery/casting/metal/${item.material}/rod_sand_cast`
                });
                event.recipes.tconstruct
                    .casting_table(Item.of(`#forge:rods/${item.material}`), item.fluid_id, 45)
                    .multiUseCast('rod')
                    .coolingTime(40)
                    .id(`mbm2:smeltery/casting/metal/${item.material}/rod_gold_cast`);
                event.recipes.tconstruct
                    .casting_table(Item.of(`#forge:rods/${item.material}`), item.fluid_id, 45)
                    .singleUseCast('rod')
                    .coolingTime(40)
                    .id(`mbm2:smeltery/casting/metal/${item.material}/rod_sand_cast`);
                ...
                global.thermalChilling(
                    ...
                    5000,
                    `thermal:machines/chiller/chiller_${item.material}_rod`
                );
            ...
            if (Item.of(`#forge:scaffoldings/${item.material}`) != null) {
                if (Item.of(`#forge:rods/${item.material}`) != null) {
                    event
                        .shaped(`2x #forge:scaffoldings/${item.material}`, ['RRR', 'R R', 'RRR'], {
                            R: `#forge:rods/${item.material}`
                        })
                    ...
                    console.log(
                        `${item.material}` + 'needs rods enabled to make a scaffolding recipe'
                    );
                }
                ...
                if (Item.of(`#forge:frame_boxs/${item.material}`) != null) {
                    event
                        .shaped(`#forge:frame_boxs/${item.material}`, ['BPB', 'PSP', 'BPB'], {
                            S: `#forge:scaffoldings/${item.material}`,
                    ...
                    ////////////////HULL CASINGS///////////////
                    if (
                        Item.of(`#forge:hull_casings/${item.material}`) != null &&
                        Item.of(`#forge:hull_panels/${item.material}`) != null
                    ) {
                        event
                            .shaped(`#forge:hull_casings/${item.material}`, ['BPB', 'PFP', 'BPB'], {
            ...
            if (Item.of(`#forge:sprockets/${item.material}`) != null) {
                event.recipes.botania.mana_infusion(
                    Item.of(`#forge:rings/${item.material}`),
                    `#forge:rods/${item.material}`,
                    2000
                );
                event
                    .shaped(`#forge:sprockets/${item.material}`, [' R ', 'RRR', ' R '], {
        ...
        if (Item.of(`#forge:bolts/${item.material}`) != null) {
            ...
            if (item.fluid_id != null) {
                if (item.tier <= 2) {
                    event.recipes.tconstruct
                        .casting_table(Item.of(`#forge:bolts/${item.material}`), item.fluid_id, 180)
                        .cast('#forge:rods/iron')
                        .coolingTime(60)
                        .consumeCast()
                        .id(`mbm2:smeltery/casting/metal/${item.material}/bolt`); //.switchSlots()
                } else {
                    event.recipes.tconstruct
                        .casting_table(Item.of(`#forge:bolts/${item.material}`), item.fluid_id, 180)
                        .cast('#forge:rods/steel')
                        .coolingTime(60)
                        .consumeCast()
                        .id(`mbm2:smeltery/casting/metal/${item.material}/bolt`); //.switchSlots()
                }
            } else {
                console.log(`[MM2 ERROR] ${item.material}` + 'needs a fluid_id or bolts casting');
            }

            //Sintering Bolts
            event.recipes.multiblocked
                .multiblock('sintering')
                .setChance(0)
        ...
        if (Item.of(`#forge:wires/${item.material}`) != null) {
            //Hand Crafting
            event
                .shapeless(`#forge:wires/${item.material}`, [
                    `#forge:plates/${item.material}`,
                    Item.of('immersiveengineering:wirecutter').ignoreNBT()
                ])
                .id(`immersiveengineering:crafting/wire_${item.material}`);

            //Create Wire
            global.createRolling(
                event,
                `#forge:wires/${item.material}`,
                1,
                `forge:plates/${item.material}`,
                `createaddition:rolling/${item.material}_plate`
            );

            //Immersive Wire
            event.recipes
                .immersiveengineeringMetalPress(
                    `2x #forge:wires/${item.material}`,
                    `#forge:ingots/${item.material}`,
                    'immersiveengineering:mold_wire'
                )
                .id(`immersiveengineering:metalpress/wire_${item.material}`);

            if (item.fluid_id != null) {
                //Wire Casing
                event.remove({
                    id: `tconstruct:smeltery/casting/metal/${item.material}/wire_gold_cast`
                });
                event.remove({
                    id: `tconstruct:smeltery/casting/metal/${item.material}/wire_sand_cast`
                });
                event.recipes.tconstruct
                    .casting_table(Item.of(`#forge:wires/${item.material}`), item.fluid_id, 45)
                    .multiUseCast('wire')
                    .coolingTime(40)
                    .id(`mbm2:smeltery/casting/metal/${item.material}/wire_gold_cast`);
                event.recipes.tconstruct
                    .casting_table(Item.of(`#forge:wires/${item.material}`), item.fluid_id, 45)
                    .singleUseCast('wire')
                    .coolingTime(40)
                    .id(`mbm2:smeltery/casting/metal/${item.material}/wire_sand_cast`);
            ...
            if (Item.of(`#forge:spools/${item.material}`) != null) {
                event
                    .shaped(`#forge:spools/${item.material}`, [' W ', 'WSW', ' W '], {
                        W: `#forge:wires/${item.material}`,
            ...
            if (Item.of(`#forge:wire_coils/${item.material}`) != null) {
                if (item.tier <= 2) {
                    event
                        .shaped(`#forge:wire_coils/${item.material}`, [' W ', 'WSW', ' W '], {
                            S: `#forge:rods/iron`,
            ...
            if (Item.of(`#forge:coils/${item.material}`) != null) {
                if (item.tier <= 2) {
                    event
                        .shaped(`#forge:coils/${item.material}`, ['PPP', 'CCC', 'PPP'], {
        ...
        if (Item.of(`#forge:dusts/${item.material}`) != null) {
            if (item.fluid_id != null && item.ore == false) {
                global.tinkersMeltingPlain(
                    ...
                    item.fluid_id,
                    90,
                    Item.of(`#forge:dusts/${item.material}`).toJson(),
                    800,
                    142,
                    `mbm2:smeltery/melting/${item.material}_dust`
                );
            }
            if (item.type == 'gem') {
                event.recipes
                    ...
                    .id(`mbm2:create/crushing/${item.material}_${item.type}`);
                event.recipes
                    .immersiveengineeringCrusher(
                        `#forge:dusts/${item.material}`,
                        `#forge:gems/${item.material}`
                    )
                    .id(`mbm2:immersive/crushing/${item.material}_${item.type}`);
                event.recipes
                    .mekanismCrushing(
                        `#forge:dusts/${item.material}`,
                        `#forge:gems/${item.material}`
                    )
                    .id(`mbm2:mekanism/crushing/${item.material}_${item.type}`);
            } else if (item.type == 'stone') {
                event.recipes
                    .createMilling([`#forge:dusts/${item.material}`], [`#forge:${item.material}`])
                    .id(`mbm2:create/crushing/${item.material}_${item.type}`);
                event.recipes
                    .immersiveengineeringCrusher(
                        `#forge:dusts/${item.material}`,
                    ...
                    .id(`mbm2:immersive/crushing/${item.material}_${item.type}`);
                event.recipes
                    .mekanismCrushing(`#forge:dusts/${item.material}`, `#forge:${item.material}`)
                    .id(`mbm2:mekanism/crushing/${item.material}_${item.type}`);
            } else {
                if (
                    Item.of(`#forge:ingots/${item.material}`) != null &&
        ...
        if (Item.of(`#forge:fine_dusts/${item.material}`) != null) {
            event.recipes.multiblocked
                .multiblock('powder_mill')
                .inputItem(`#forge:ingots/${item.material}`)
                .duration(200)
                .outputItem(`#forge:fine_dusts/${item.material}`)
                .setPerTick(true)
                .inputFE(1000 * item.tier);
        }

        if (Item.of(`#forge:ingots/${item.material}`) != null) {
            //Ingots
            if (item.itemParts.includes('nugget')) {
                event.shapeless(`9x #forge:nuggets/${item.material}`, [
                    `#forge:ingots/${item.material}`
                ]);
                event.shaped(`#forge:ingots/${item.material}`, ['NNN', 'NNN', 'NNN'], {
        ...
        if (item.blockParts.includes('storage_block')) {
            if (item.type == 'gem') {
                //Blocks
                event.shapeless(`9x #forge:gems/${item.material}`, [
                    `#forge:storage_blocks/${item.material}`
                ]);
                event.shaped(`#forge:storage_blocks/${item.material}`, ['III', 'III', 'III'], {
                    I: `#forge:gems/${item.material}`
                });
            } else {
                //Blocks
                event.shapeless(`9x #forge:ingots/${item.material}`, [
                    `#forge:storage_blocks/${item.material}`
                ]);
                event.shaped(`#forge:storage_blocks/${item.material}`, ['III', 'III', 'III'], {
                    I: `#forge:ingots/${item.material}`
        ...
        if (item.fluid_id != null && (item.type == 'base_metal' || item.type == 'alloy')) {
            if (Item.of(`#forge:ingots/${item.material}`) != null) {
                //Ingot Casting
                event.remove({
                    id: `tconstruct:smeltery/casting/metal/${item.material}/ingot_gold_cast`
                });
                event.remove({
                    id: `tconstruct:smeltery/casting/metal/${item.material}/ingot_sand_cast`
                });
                event.recipes.tconstruct
                    .casting_table(Item.of(`#forge:ingots/${item.material}`), item.fluid_id, 90)
                    .multiUseCast('ingot')
                    .coolingTime(60)
                    .id(`mbm2:smeltery/casting/metal/${item.material}/ingot_gold_cast`);
                event.recipes.tconstruct
                    .casting_table(Item.of(`#forge:ingots/${item.material}`), item.fluid_id, 90)
                    .singleUseCast('ingot')
                    .coolingTime(60)
                    .id(`mbm2:smeltery/casting/metal/${item.material}/ingot_sand_cast`);
            ...
            if (Item.of(`#forge:nuggets/${item.material}`) != null) {
                //Nugget Casting
                event.remove({
                    id: `tconstruct:smeltery/casting/metal/${item.material}/nugget_gold_cast`
                });
                event.remove({
                    id: `tconstruct:smeltery/casting/metal/${item.material}/nugget_sand_cast`
                });
                event.recipes.tconstruct
                    .casting_table(Item.of(`#forge:nuggets/${item.material}`), item.fluid_id, 90)
                    .multiUseCast('nugget')
                    .coolingTime(10)
                    .id(`mbm2:smeltery/casting/metal/${item.material}/nugget_gold_cast`);
                event.recipes.tconstruct
                    .casting_table(Item.of(`#forge:nuggets/${item.material}`), item.fluid_id, 90)
                    .singleUseCast('nugget')
                    .coolingTime(10)
                    .id(`mbm2:smeltery/casting/metal/${item.material}/nugget_sand_cast`);
            ...
            if (Item.of(`#forge:storage_blocks/${item.material}`) != null) {
                ...
                global.casingBasin(
                    ...
                    810,
                    180,
                    `mbm2:smeltery/casting/metal/${item.material}/block`
                );
            }
        }
        if (item.raw_ore) {
            //Raw Blocks
            event
                .shapeless(`9x #forge:raw_materials/${item.material}`, [
                    `#forge:storage_blocks/raw_${item.material}`
                ])
                .id(`mbm2:raw_${item.material}_unpacking`);
            event
                .shaped(`#forge:storage_blocks/raw_${item.material}`, ['III', 'III', 'III'], {
        ...
        if (item.fluid_id != null) {
            for (var part in partAmounts) {
                if (Item.of(`#forge:${part}s/${item.material}`) != null) {
                    event.recipes.thermal
                        .crucible(
                            Fluid.of(item.fluid_id, partAmounts[part]),
                            `#forge:${part}s/${item.material}`
                        )
                        .id(`mbm2:crucible/${item.material}_${part}`);
                    if (item.fluid_id.includes('kubejs')) {
                        global.tinkersMeltingPlain(
                            event,
                            item.fluid_id,
                            partAmounts[part],
                            Item.of(`#forge:${part}s/${item.material}`).toJson(),
                            800,
                            90,
                            `mbm2:smeltery/melting/${item.material}_${part}`
                        );
                    }
                }
            }
        ...
        if (Item.of(`chemlib:${item.material}`) != null && item.type != 'element') {
            ...
            global.alchemistryCompacting(
                event,
                Item.of(`#forge:dusts/${item.material}`).toJson(),
                Item.of(`chemlib:${item.material}`, 16).toJson(),
                `mbm2:alchemistry/${item.material}_compacting`
            );
        }
        //Compound Ingots
        ...
        if (Item.of(`#forge:part_bundles/${item.material}`) != null) {
            event
                .shapeless(Item.of(`#forge:part_bundles/${item.material}`), [
                    'minecraft:bundle',
