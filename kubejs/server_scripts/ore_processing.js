	...
	global.newMaterialParts.forEach((item) => {
		...
		if (item.ore) {
			...
			if (item.type == 'base_metal') {
				//Make some ingots
				if (item.tier <= 1) {
					event.smelting(`#forge:ingots/${item.material}`, `#forge:grits/${item.material}`).id(`kubejs:smelting/grit_${item.material}`)
					event.blasting(`#forge:ingots/${item.material}`, `#forge:grits/${item.material}`).id(`kubejs:blasting/grit_${item.material}`)
					event.smelting(`#forge:ingots/${item.material}`, `#forge:dusts/${item.material}`).id(`kubejs:smelting/dust_${item.material}`)
					event.blasting(`#forge:ingots/${item.material}`, `#forge:dusts/${item.material}`).id(`kubejs:blasting/dust_${item.material}`)
					event.smelting(`#forge:ingots/${item.material}`, `#create:crushed_ores/${item.material}`).id(`kubejs:smelting/crushed_${item.material}`)
					event.blasting(`#forge:ingots/${item.material}`, `#create:crushed_ores/${item.material}`).id(`kubejs:blasting/crushed_${item.material}`)
					event.smelting(`#forge:ingots/${item.material}`, `#forge:washed_ores/${item.material}`).id(`kubejs:smelting/washed_${item.material}`)
					event.blasting(`#forge:ingots/${item.material}`, `#forge:washed_ores/${item.material}`).id(`kubejs:blasting/washed_${item.material}`)
				}
				if (item.tier >= 1) {
				...
				event.recipes.createCrushing([
				  ...
				  ], `#forge:raw_materials/${item.material}`).id(`kubejs:crushing/raw_${item.material}`)
				//Create Washing
				event.recipes.createSplashing([`#forge:washed_ores/${item.material}`, Item.of(`#forge:washed_ores/${item.material}`).withChance(0.75), Item.of('gravel').withChance(0.25)], `#create:crushed_ores/${item.material}`).id(`kubejs:washing/crushed_${item.material}`)
				}
			}
			if (item.type == 'compound_ore') {
					...
					if (Item.of(`#forge:ingots/${item.components[0]}`) != null) {
						event.smelting(`#forge:ingots/${item.components[0]}`, `#forge:raw_materials/${item.material}`).id(`kubejs:smelting/raw_${item.material}`)
						event.smelting(`#forge:ingots/${item.components[0]}`, `#forge:dusts/${item.material}`).id(`kubejs:smelting/dust_${item.material}`)
						event.blasting(`#forge:ingots/${item.components[0]}`, `#forge:raw_materials/${item.material}`).id(`kubejs:blasting/raw_${item.material}`)
						event.blasting(`#forge:ingots/${item.components[0]}`, `#forge:dusts/${item.material}`).id(`kubejs:blasting/dust_${item.material}`)
						event.smelting(`#forge:ingots/${item.components[0]}`, `#forge:grits/${item.material}`).id(`kubejs:smelting/grit_${item.material}`)
						event.blasting(`#forge:ingots/${item.components[0]}`, `#forge:grits/${item.material}`).id(`kubejs:blasting/grit_${item.material}`)
					} else {
					...
					if (item.gem_components != null) {
						if (Item.of(`#forge:gems/${item.gem_components[0]}`) != null) {
							global.pneumaticcraftThermoPlant(event, Item.of(`#forge:gems/${item.gem_components[0]}`), Ingredient.of(`#forge:dusts/${item.material}`), {"type": "pneumaticcraft:fluid", "fluid": "kubejs:gemstone_catalyst_mixture", "amount": 250}, 423, 4, 0.75, 1.0, false, `kubejs:thermo_plant/${item.gem_components[0]}_from_${item.material}`)
						} else {
